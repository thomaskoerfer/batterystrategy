#!/usr/bin/env python3
"""Evaluate stochastic optimizer decisions against finalized actuals."""
...
TRACE_SCHEMAS = frozenset({4, 5, 7, 8})
...
def load_traces(root: str | Path, start_ms: int, end_ms: int) -> list[dict]:
    """Load complete scenario traces from the shadow and cutover schemas."""
    traces = []
    root = Path(root)
    if not root.exists():
        return traces
...
def score_decisions(
    ...
    *,
    ...
    for trace in traces[:: max(1, stride)]:
        if isinstance(trace.get("optimizer_plan"), dict):
...
def _optimizer_evaluation(trace: dict) -> dict:
    """Return cutover diagnostics while retaining offline schema-4 readability."""
...
def summarize(
    ...
    *,
    ...
    report = {
        ...
        "completed_optimizer_vintages": sum(
            _optimizer_evaluation(item).get("status")
            in {"completed", "authoritative_stochastic"}
            for item in traces
        ),
