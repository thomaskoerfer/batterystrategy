...
except ImportError:  # pragma: no cover - allows importing constants in unit tests without HA.
    vol = None
    config_entries = None
    selector = None
...
if config_entries is not None:
    ...
    def _number_selector(minimum: float, maximum: float, step: float, unit: str | None = None, mode: str = "box"):
        """Return a number selector."""
        if selector is None:
            return vol.Coerce(float)
        return selector.NumberSelector(
            selector.NumberSelectorConfig(min=minimum, max=maximum, step=step, unit_of_measurement=unit, mode=mode)
        )
    ...
    class BatteryStrategyOptionsFlow(config_entries.OptionsFlow):
        ...
        async def async_step_strategy(self, user_input=None):
            ...
            schema = vol.Schema(
                {
                    vol.Required("strategy_section", default="mode"): _select_label_selector(
                        [
            ...
            schema = vol.Schema(
                {
                    ...
                    vol.Required("grid_charging", default=options.get("grid_charging", GRID_CHARGING_OFF)): _select_selector(
                        [GRID_CHARGING_OFF, GRID_CHARGING_PRICE_SENSITIVE]
                    ),
                    vol.Required("discharge", default=options.get("discharge", DISCHARGE_LOAD)): _select_selector(
        ...
        async def async_step_strategy_battery(self, user_input=None):
            ...
            schema = vol.Schema(
                {
                    vol.Required("min_soc_pct", default=options.get("min_soc_pct", 10.0)): _number_selector(0, 100, 1, "%"),
                    vol.Required("max_soc_pct", default=options.get("max_soc_pct", 100.0)): _number_selector(0, 100, 1, "%"),
                    vol.Required(
                        "max_charge_power_w",
        ...
        async def async_step_strategy_manual(self, user_input=None):
            ...
            schema = vol.Schema(
                {
                    vol.Required("manual_mode", default=options.get("manual_mode", MANUAL_OFF)): _select_selector(
                        [MANUAL_OFF, MANUAL_CHARGE, MANUAL_DISCHARGE]
                    ),
                    vol.Required("manual_power_w", default=options.get("manual_power_w", 0.0)): _number_selector(0, 2400, 50, "W"),
                    vol.Required("manual_duration_min", default=options.get("manual_duration_min", 30)): _number_selector(0, 240, 15, "min"),
                }
            )
    ...
    def _entity_schema(data: dict[str, str], include_reference: bool) -> vol.Schema:
        ...
        schema = {
            ...
            vol.Required(CONF_BATTERY_PROFILE, default=data.get(CONF_BATTERY_PROFILE, BATTERY_PROFILE_ZENDURE)): _select_selector(
                [BATTERY_PROFILE_ZENDURE, BATTERY_PROFILE_GENERIC]
            ),
            vol.Optional(CONF_SIGNED_GRID_POWER_ENTITY, default=data.get(CONF_SIGNED_GRID_POWER_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_GRID_L1_ENTITY, default=data.get(CONF_GRID_L1_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_GRID_L2_ENTITY, default=data.get(CONF_GRID_L2_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_GRID_L3_ENTITY, default=data.get(CONF_GRID_L3_ENTITY, "")): _entity_selector(["sensor"]),
            ...
            vol.Optional(CONF_ZENDURE_OUTPUT_PACK_POWER_ENTITY, default=data.get(CONF_ZENDURE_OUTPUT_PACK_POWER_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_ZENDURE_PACK_INPUT_POWER_ENTITY, default=data.get(CONF_ZENDURE_PACK_INPUT_POWER_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_ZENDURE_OUTPUT_HOME_POWER_ENTITY, default=data.get(CONF_ZENDURE_OUTPUT_HOME_POWER_ENTITY, "")): _entity_selector(["sensor"]),
            vol.Optional(CONF_ZENDURE_GRID_INPUT_POWER_ENTITY, default=data.get(CONF_ZENDURE_GRID_INPUT_POWER_ENTITY, "")): _entity_selector(["sensor"]),
