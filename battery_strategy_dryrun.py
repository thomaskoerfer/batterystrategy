...
PV_SURPLUS_START_AVG_W = 80.0
PV_SURPLUS_MIN_SAMPLE_W = 50.0
PV_SURPLUS_REQUIRED_COUNT = 3
PV_SURPLUS_WINDOW_SAMPLES = 4
...
def forecast_pv_w_for_slot(samples, target_dt, weather_factor, fallback_w=0.0, slot_bias=1.0):
    ...
    return max(0.0, pv_w * weather_factor * clamp(slot_bias, 0.6, 1.5))
...
def build_virtual_plan(intervals, samples, start_energy_kwh, weather_factor, forecast_tomorrow_kwh, load_bias, load_bias_slots, pv_bias_slots, initial_mode=0, weather_hourly=None, pv_now_actual_w=None, now_local=None, pv_global_bias=1.0, eex_days=None, inventory_discharge_floor_ct=None):
    ...
    if inventory_discharge_floor_ct is not None:
        discharge_floor_ct = max(float(inventory_discharge_floor_ct), float(discharge_floor_ct or 0.0))
    ...
    for t in range(n_slots):
        ...
        for i, e_now in enumerate(energies):
            min_e_next = max(MIN_E_KWH, e_now - min(max_discharge_delta_e, net_pos / ETA_D))
                    ...
                    if charge_from_grid > 1e-9:
                        ...
                        if later_cheaper_charge_capacity_kwh > 1e-9:
                            ...
                            if later_cheaper_charge_capacity_kwh >= (additional_profitable_charge_in_kwh - 1e-6):
                                continue
                    grid_import = max(0.0, net_pos - discharge_out) + charge_from_grid
    ...
    for t in range(n_slots, 0, -1):
        ...
        sl = slots[t - 1]
        grid_export = max(0.0, sl["surplus_kwh"] - charge_in)
...
def suppress_uneconomic_micro_cycles(points, start_energy_kwh):
    ...
    for idx, p in enumerate(points):
        if float(p.get("discharge_fc_w", 0.0)) <= 1.0:
            continue
        recent_charge_prices = [
            float(points[j].get("price_ct", 0.0))
            for j in range(max(0, idx - MICROCYCLE_LOOKBACK_SLOTS), idx)
            if float(points[j].get("charge_fc_w", 0.0)) > 1.0
        ]
        if not recent_charge_prices:
            continue
        recent_charge_avg_ct = sum(recent_charge_prices) / len(recent_charge_prices)
    ...
    for p in points:
        ...
        discharge_out = min(req_discharge_out, MAX_E_SLOT_KWH, net_pos_kwh, max_discharge_out)

        p["soc_pct"] = round((energy / CAP_KWH) * 100.0, 2)
        p_bat_w = ((charge_in - discharge_out) / SLOT_H) * 1000.0
        grid_import = max(0.0, net_pos_kwh - discharge_out) + max(0.0, charge_in - surplus_kwh)
        grid_export = max(0.0, surplus_kwh - charge_in)
...
def compute_actual_savings(today_iso):
    daily = {}
    now_ts = dt.datetime.now(dt.timezone.utc).timestamp()
    cutoff = now_ts - ACTUAL_SAVINGS_DAYS * 86400
    series_map = fetch_sensor_series_many(
        [
            "sensor.electricity_price_heerlener_strasse_300",
    ...
    price_series = series_map["sensor.electricity_price_heerlener_strasse_300"]
    input_series = series_map["sensor.battery_input"]
    output_series = series_map["sensor.battery_output"]
    grid_import_series = series_map["sensor.battery_strategy_grid_import_live_power_now"]
    grid_export_series = series_map["sensor.battery_strategy_grid_export_live_power_now"]
    battery_power_series = series_map["sensor.battery_strategy_battery_in_out_live_power"]
    if not price_series or (len(input_series) < 2 and len(output_series) < 2):
        return daily, 0.0, 0.0, 0.0, None

    # Use the battery's cumulative energy counters directly:
    # - battery_input: charged energy (kWh)
    # - battery_output: discharged energy (kWh)
    # Cost/credit is valued at the contemporaneous Tibber price.
    # `sensor.electricity_price_*` is typically EUR/kWh, but some setups may expose ct/kWh.
    # We detect and normalize once per run.
    max_delta_kwh = 8.0
    price_vals = [float(v) for _, v in price_series]
    price_median = statistics.median(price_vals) if price_vals else 0.0
    # Heuristic: values > 2 are almost certainly ct/kWh (e.g. 25..45),
    # otherwise assume EUR/kWh (e.g. 0.25..0.45).
    price_is_ct = price_median > 2.0
    price_index = build_series_index(price_series)
    grid_import_index = build_series_index(grid_import_series)
    grid_export_index = build_series_index(grid_export_series)
    battery_power_index = build_series_index(battery_power_series)
    inventory_lots = []
    ...
    def estimate_charge_split(delta_kwh, ts):
        if delta_kwh <= 0:
            return 0.0, 0.0

        gi = max(0.0, float(indexed_value_at_or_before(grid_import_index, ts) or 0.0))
        ge = max(0.0, float(indexed_value_at_or_before(grid_export_index, ts) or 0.0))
        bat = float(indexed_value_at_or_before(battery_power_index, ts) or 0.0)
        charge_w = max(0.0, -bat)
        export_without_battery_w = max(0.0, -(gi - ge + bat))

        if charge_w <= 1.0:
            pv_ratio = 1.0 if export_without_battery_w > 1.0 else 0.0
        else:
            pv_ratio = min(1.0, export_without_battery_w / charge_w)

        pv_kwh = delta_kwh * pv_ratio
        grid_kwh = max(0.0, delta_kwh - pv_kwh)
        return grid_kwh, pv_kwh

    def process_counter(series, is_charge):
        if len(series) < 2:
            return
        prev_ts, prev_v = float(series[0][0]), float(series[0][1])
        for ts, val in series[1:]:
            ts = float(ts)
            val = float(val)
            delta = val - prev_v
            prev_ts, prev_v = ts, val
            if delta <= 0:
                # Counter reset or no movement.
                continue
            if delta > max_delta_kwh:
                # Guardrail against corrupted jumps.
                continue

            price_ct = indexed_value_at_or_before(price_index, ts)
            if price_ct is None:
                continue
            price_eur = (float(price_ct) / 100.0) if price_is_ct else float(price_ct)
            day = local_dt_from_ts(ts).date().isoformat()
            rec = ensure_day(day)

            if is_charge:
                charge_grid_kwh, charge_pv_kwh = estimate_charge_split(delta, ts)
                rec["charge_grid_kwh"] += charge_grid_kwh
                rec["charge_pv_kwh"] += charge_pv_kwh
                charge_cost_eur = charge_grid_kwh * price_eur
                rec["charge_cost_eur"] += charge_cost_eur
                rec["saving_eur"] -= charge_cost_eur
                deliverable_kwh = max(0.0, (charge_grid_kwh + charge_pv_kwh) * ETA_RT)
                if deliverable_kwh > 1e-9:
                    inventory_lots.append(
                        {
                            "deliverable_kwh": deliverable_kwh,
                            "cost_eur": charge_cost_eur,
                        }
                    )
            else:
                rec["discharge_used_kwh"] += delta
                discharge_credit_eur = delta * price_eur
                rec["discharge_credit_eur"] += discharge_credit_eur
                rec["saving_eur"] += discharge_credit_eur
                remaining = delta
                while remaining > 1e-9 and inventory_lots:
                    lot = inventory_lots[0]
                    take = min(remaining, float(lot["deliverable_kwh"]))
                    if take <= 1e-9:
                        inventory_lots.pop(0)
                        continue
                    lot_deliverable = max(1e-9, float(lot["deliverable_kwh"]))
                    lot_cost = float(lot["cost_eur"])
                    cost_share = lot_cost * (take / lot_deliverable)
                    lot["deliverable_kwh"] = lot_deliverable - take
                    lot["cost_eur"] = max(0.0, lot_cost - cost_share)
                    remaining -= take
                    if lot["deliverable_kwh"] <= 1e-9:
                        inventory_lots.pop(0)

    process_counter(input_series, is_charge=True)
    process_counter(output_series, is_charge=False)
    ...
    for d, rec in daily.items():
        rec["charge_grid_kwh"] = round(rec["charge_grid_kwh"], 4)
        rec["charge_pv_kwh"] = round(rec["charge_pv_kwh"], 4)
        rec["discharge_used_kwh"] = round(rec["discharge_used_kwh"], 4)
        rec["charge_cost_eur"] = round(rec["charge_cost_eur"], 4)
        rec["discharge_credit_eur"] = round(rec["discharge_credit_eur"], 4)
        rec["saving_eur"] = round(rec["saving_eur"], 4)

    today_actual = float(daily.get(today_iso, {}).get("saving_eur", 0.0))
    cumulative_actual = sum(
        float(v.get("saving_eur", 0.0)) for k, v in daily.items() if k <= today_iso
    )
    inventory_deliverable_kwh = sum(max(0.0, float(lot.get("deliverable_kwh", 0.0))) for lot in inventory_lots)
    inventory_cost_eur = sum(max(0.0, float(lot.get("cost_eur", 0.0))) for lot in inventory_lots)
    inventory_cost_ct_per_kwh = None
    if inventory_deliverable_kwh > 1e-9:
        inventory_cost_ct_per_kwh = (inventory_cost_eur / inventory_deliverable_kwh) * 100.0
    return (
        daily,
        round(today_actual, 3),
        round(cumulative_actual, 3),
        round(inventory_deliverable_kwh, 4),
        round(inventory_cost_ct_per_kwh, 3) if inventory_cost_ct_per_kwh is not None else None,
    )
...
def main():
    ...
    (
        actual_daily_savings,
        actual_today_saving,
        actual_cumulative_saving,
        actual_inventory_deliverable_kwh,
        actual_inventory_cost_ct_per_kwh,
    ) = compute_actual_savings(today)
    data["actual_daily_savings"] = actual_daily_savings
    ...
    out = {
        ...
        "actual_savings_cumulative_eur": round(actual_cumulative_saving, 3),
        ...
        "profile_48h_pv_fc_power": build_anchored_hourly_series(hourly_points, "pv_fc_w", now_ts_ms, anchor_w=max(0.0, pv_w)),
